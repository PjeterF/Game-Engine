...
void SpriteRenderer::draw(glm::vec2 position, glm::vec2 scale, float angle, Texture* texture, Animation::Division division)
{
	if (texture == nullptr)
		return;
	...
	float xOffset = division.xmin / texture->getWidth();
	float yOffset = division.ymin / texture->getHeight();
	float xScale = (division.xmax - division.xmin) / texture->getWidth();
	float yScale = (division.ymax - division.ymin) / texture->getHeight();
